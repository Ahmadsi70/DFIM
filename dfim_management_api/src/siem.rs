...
impl SiemSerializer for Rfc5424SyslogSerializer {
    ...
    fn serialize(&self, event: &DfimSiemEvent) -> String {
        ...
        format!(
            ...
            pri, ts, event.host_name, "12345", sd, event.message
        )
    }
}
...
pub fn format_batch(serializer: &dyn SiemSerializer, events: &[DfimSiemEvent]) -> String {
    match serializer.format() {
        SiemFormat::Ndjson => events
            .iter()
            .map(|e| serializer.serialize(e))
            .collect::<Vec<_>>()
            .join("\n"),
        _ => {
            let items: Vec<String> = events.iter().map(|e| serializer.serialize(e)).collect();
...
mod tests {
    ...
    fn make_event() -> DfimSiemEvent {
        DfimSiemEvent {
            ...
            event_type: "dfim.integrity.failure".into(),
            severity: "critical".into(),
            asset_id: "sha256:abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890"
                .into(),
            message: "Boot image tampered".into(),
            host_name: "dfim-edge-01".into(),
            timestamp: Utc::now(),
            outcome: "failure".into(),
            merkle_root: Some("aabbccdd".into()),
            tampered_blocks: Some(3),
            fec_corrected: Some(1),
            rollback_counter: Some(42),
            attestation_verified: Some(true),
            enforcement_action: Some("block".into()),
        }
    }
